# Table of contents

* [Overview](README.md)
* [Integrating Common Service](integrating-common-service.md)

## Central Registry

* [Overview](central-registry/overview.md)
* [CR API Details](central-registry/cr-api-details.md)

## Token Service Guide <a href="token-service" id="token-service"></a>

* [Access Token Logistics](token-service/access-token-logistics.md)
* [Token Verification](token-service/token-verification.md)
* [Code Snippets](token-service/code-snippets.md)

## ❔ faqs

* [Common Central Registry](faqs/common-central-registry.md)
* [Common Token Issuance Server](faqs/common-token-issuance-server.md)

## Reference

* [Access Token Schema](reference/access-token-schema.md)
